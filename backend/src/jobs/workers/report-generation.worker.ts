...
export class ReportGenerationWorker extends WorkerHost {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    ...
    this.logger.log(`Processing report generation job ${job.id} for business ${businessId} in format ${format}`);
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this.logger.error(`Report generation job ${job.id} failed: ${err.message}`, err.stack);
