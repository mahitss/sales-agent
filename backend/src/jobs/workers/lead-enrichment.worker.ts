...
export class LeadEnrichmentWorker extends WorkerHost {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    ...
    this.logger.log(
      `Processing Lead Enrichment job ${job.id} for lead ${leadId}`,
    );

    try {
      const result =
        await this.leadIntelligenceService.processLeadAnalysis(leadId);
      ...
      const duration = Date.now() - startTime;
      this.logger.error(
        `Lead Enrichment job ${job.id} failed: ${err.message}`,
        err.stack,
      );
