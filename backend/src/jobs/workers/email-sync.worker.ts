  ...
  async process(job: Job<any, any, string>): Promise<any> {
    ...
    try {
      if (job.name === 'sync-inbox') {
        this.logger.log(`Running inbox sync job ${job.id} for account ${accountId}`);
        ...
        this.logger.log(`Running simulated reply job ${job.id} on activity ${sentActivityId}`);
        await this.emailIntegrationService.handleSimulatedReply(accountId, sentActivityId, businessId);
      ...
      this.logger.error(`Email Sync Worker Job ${job.id} failed: ${err.message}`, err.stack);
      await this.logJob(
        job.id || 'error', 
        'email-sync', 
        job.name, 
