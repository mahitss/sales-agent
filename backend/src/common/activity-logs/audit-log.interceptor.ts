...
const ROUTE_ACTION_MAP: Array<{
  ...
  {
    method: 'POST',
    pattern: /\/integrations\/email\/send$/i,
    action: 'EMAIL_SENT',
    entity: 'EmailActivity',
    severity: 'INFO',
  },
  {
    method: 'DELETE',
    pattern: /\/integrations\/email\/accounts\/[^/]+$/i,
    action: 'EMAIL_ACCOUNT_REMOVED',
    entity: 'EmailAccount',
    severity: 'WARN',
    entityIdParam: -1,
  },
  ...
  {
    method: 'POST',
    pattern: /\/account-research$/i,
    action: 'AI_RESEARCH_STARTED',
    entity: 'AccountResearch',
    severity: 'INFO',
  },
...
export class AuditLogInterceptor implements NestInterceptor {
  ...
  private buildDescription(
    ...
    return (
      descMap[action] || `${action.replace(/_/g, ' ')} performed by ${who}`
    );
  }
