...
export class AICostService {
  ...
  private readonly OUTPUT_TOKEN_RATE = 0.30 / 1_000_000;
  ...
  async logUsage(
    ...
    try {
      const calculatedCost =
        promptTokens * this.INPUT_TOKEN_RATE + completionTokens * this.OUTPUT_TOKEN_RATE;
      ...
      this.logger.error(`Failed to write AI cost log: ${err.message}`, err.stack);
    }
  }
  ...
  async getCostStats(businessId: string) {
    ...
    const totalCompletionTokens = logs.reduce((acc, l) => acc + l.completionTokens, 0);
