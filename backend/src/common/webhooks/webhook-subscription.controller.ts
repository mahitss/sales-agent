...
export class WebhookSubscriptionController {
  ...
  async create(
    ...
    if (req.user.role !== 'ADMIN') {
      throw new ForbiddenException('Only workspace administrators can subscribe to outbound webhooks');
    }
    if (!body.url || !body.events || !Array.isArray(body.events)) {
      throw new BadRequestException('Webhook URL and array of events are required');
    }
    return this.webhookService.createSubscription(req.user.businessId, body.url, body.events);
  }
  ...
  async delete(@Param('id') id: string, @Req() req) {
    ...
    if (req.user.role !== 'ADMIN') {
      throw new ForbiddenException('Only workspace administrators can delete webhook subscriptions');
    }
