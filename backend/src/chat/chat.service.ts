...
export class ChatService {
  ...
  async sendMessage(dto: SendMessageDto) {
    ...
    try {
      ...
      if (cached) {
        ...
        await this.postProcessAIResponse(
          lead,
          conversation,
          businessId,
          messageHistory,
    ...
    if (!isCircuitAllowed) {
      this.logger.warn(
        'Circuit breaker is OPEN. Falling back to mock response.',
      );
      ...
      while (attempt < maxRetries) {
        try {
          ...
          attempt++;
          console.error(
            `Attempt ${attempt} to connect to AI Service chat failed: ${err.message}`,
          );
  ...
  async streamMessage(dto: SendMessageDto, res: any) {
    ...
    while (attempt < maxRetries) {
      try {
        ...
        attempt++;
        console.error(
          `Attempt ${attempt} to connect to AI Service stream failed: ${err.message}`,
        );
        if (attempt >= maxRetries) {
          this.recordFailure();
          console.warn(
            'AI Service streaming failed. Falling back to mock stream.',
          );
          const fallback = this.getFallbackAIResponse(message);
  ...
  private async postProcessAIResponse(
    ...
    if (
      aiResponse.extracted_name &&
      (lead.name === 'Anonymous Visitor' || !lead.name)
    ) {
    ...
    if (
      aiResponse.engagement_score !== undefined &&
      aiResponse.engagement_score !== null
    ) {
    ...
    if (
      aiResponse.extracted_appointment_date &&
      aiResponse.extracted_appointment_time
    ) {
    ...
    if (statsChanged) {
      await this.redisService
        .del(`business:${businessId}:lead-stats`)
        .catch(() => {});
    }
    ...
    await this.jobsService
      .addLeadEnrichmentJob(lead.id, businessId)
      .catch(() => {});
  }
  ...
  private recordFailure() {
    ...
    if (ChatService.failureCount >= ChatService.failureThreshold) {
      ...
      ChatService.lastStateChange = Date.now();
      this.logger.warn(
        `Circuit breaker opened. Too many consecutive failures (${ChatService.failureCount}).`,
      );
    }
  }
  ...
  private getFallbackAIResponse(message: string) {
    return {
      response:
        "I'm experiencing connectivity issues. May I get your name and email so our human agent can reach out?",
      intent: 'Support',
      extracted_name: null,
  ...
  async simulateIncomingMessage(dto: {
    ...
    const { businessId, channel, message, leadName, leadPhone, leadEmail } =
      dto;
